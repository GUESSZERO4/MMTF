...
from utils import (ProgressBar, get_vocabulary, get_entities, SeqEntityScore, json_to_text, MSAProcessor,
                   get_msa_vocabulary, MSADataset, get_model_size)
...
def loss_fn(outputs, targets):
    # return torch.nn.MultiLabelMarginLoss(reduction='mean')(outputs, targets.long())
    return torch.nn.BCEWithLogitsLoss()(outputs, targets)

# multiclass AUROC
def auROC(y_pred, y_true):
    Label_list = ['Atelectasis', 'Cardiomegaly', 'Effusion', 'Infiltration', 'Mass', 'Nodule', 'Pneumonia',
              'Pneumothorax', 'Consolidation', 'Edema', 'Emphysema', 'Fibrosis', 'Pleural_Thickening', 'Hernia']
    map_label = {i: label for i, label in enumerate(Label_list)}
    row, col = np.array(y_true).shape
    temp = []
    for i in range(col):
        ROC = roc_auc_score(y_true[:, i], y_pred[:, i], average='micro', sample_weight=None)
        # print("%d th AUROC: %f" % (i, ROC))
        print("%s *AUROC*: %f" % (map_label[i], ROC))
        temp.append(ROC)
    ROC = 0
    for i in range(col):
        ROC += float(temp[i])
...
def calculate_acuracy_mode(model_pred, labels):
    accuracy_th = 0.5
...
def macfscore(y_pred, y_true, threash=0.5, type='macro'):
    # print("all_preds:", y_pred[0])
    y_pred = np.where(y_pred > threash, 1, 0)
    # print("y_pred:", y_pred[0])
    return f1_score(y_true, y_pred, average=type)
...
def evaluate_msa(args, model, eval_dataset, LOSS_TI, cross_entropy, prefix="", flag=None):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            loss_task = loss_fn(logits, batch[4].float()) * args.max_seq_length
            # print('loss_ti:', loss_ti, ', loss_it:', loss_it, ', loss_task:', loss_task)
            tmp_eval_loss = calculate_multi_loss(
                loss_it=loss_it, loss_ti=loss_ti, loss_task=loss_task, args=args)
            pbar(step, {'loss': tmp_eval_loss.item()})
    ...
    AUCROC = auROC(all_preds, all_labels)
    macro_f1 = macfscore(all_preds, all_labels)
    acc = hamming_score(all_labels, all_preds)
    # precision, recall = calculate_acuracy_mode(Sigmoid_fun(all_preds), all_labels)
    results = {'AUCROC': AUCROC, 'macro_f1': macro_f1, 'hamm_score': acc}
    logger.info("\n")
    logger.info("***** Eval results %s *****", prefix)
    info = "-".join([f' {key}: {value:.4f} ' for key,
                    value in results.items()])
...
def main():
    ...
    parser.add_argument("--resnet_pretrained_dir",
                        default='./resnet152-b121ed2d.pth',
                        type=str,
    ...
    if args.task == 'twitter15':
        ...
        if args.data_dir is None:
            args.data_dir = '/home/hzy/lixiaoyu/MuSE-main/NIH Chest-ray14/NIH Chest-ray14/test'
        if args.image_dir is None:
            args.image_dir = '/home/hzy/lixiaoyu/MuSE-main/NIH Chest-ray14/NIH Chest-ray14/images'
        # text_checkpoint = 'your url'
        # image_checkpoint = 'your url'
    ...
    if args.bert_type == 'uncased':
        bert_dir = '/home/hzy/lixiaoyu/MuSE-main/shenzheng_covid19/bert-base-uncased'
        args.use_xlmr = False
    elif args.bert_type == 'cased':
        bert_dir = './bert-base-cased'
        args.use_xlmr = False
    ...
    logger.info("Finish loading model [%s] parameters", get_model_size(
        model))
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:  # Complete each gradient accumulation and make a gradient cut
                        ...
                        if args.local_rank == -1:
                            ...
                            if args.task not in ['mvsa-single', 'mvsa-multiple']:
                                ...
                                results, eval_loss = evaluate_msa(
                                    args, model, dev_dataset, LOSS_TI, cross_entropy)
                                eval_results = results['hamm_score']
